        ...
        case WT_Menu:
        {
            switch (primitive)
            {
                case Generic::Frame:
                {
                    ...
                    QColor light = _helper.calcLightColor(_helper.backgroundTopColor(color));
                    QColor dark = _helper.calcDarkColor(color);

                    p->setBrush(Qt::NoBrush);

                    if (0) { // TODO make option
                        QColor shadow = _helper.calcShadowColor(color); // wrong, use kwin shadow color
                        p->setPen(OxygenHelper::alphaColor(shadow, 0.1));
                    ...
                    light = _helper.calcLightColor(_helper.backgroundBottomColor(color));
