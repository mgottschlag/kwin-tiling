...
#include "kde-system-monitor-daemon.h"
...
{
  fprintf( out, "kde-system-monitor-daemon %s\n"
  ...
  if ( ( file = fopen( LockFile, "w+" ) ) != NULL ) {
    ...
    if ( fcntl( fileno( file ), F_SETLK, &lock ) < 0 ) {
      if ( ( errno == EACCES ) || ( errno == EAGAIN ) ) {
        log_error( "kde-system-monitor-daemon is running already" );
        fprintf( stderr, "kde-system-monitor-daemon is running already\n" );
        fclose( file );
        return -1;
      }
    }
...
int createServerSocket()
{
   ...
   */
  if ( SocketPort == -1 ) {
    if ( ( service = getservbyname( "kde-system-monitor-daemon", "tcp" ) ) == NULL ) {
      /**
...
int main( int argc, char* argv[] )
{
  ...
  struct timeval last;
    ...
    kmem group, kde-system-monitor-daemon will crash because of permission problems
  ...
  if ( geteuid() != 0 ) {
    ...
    if ( getegid() != grentry->gr_gid ) {
      fprintf( stderr, "kde-system-monitor-daemon can't be started because of permission conflicts!\n"
