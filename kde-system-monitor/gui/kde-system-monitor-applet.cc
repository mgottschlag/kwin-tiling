...
extern "C"
{
  ...
  {
    KGlobal::locale()->insertCatalog( "kde-system-monitor" );
    return new kde-system-monitor-applet( configFile, Plasma::Normal,
                                Plasma::Preferences, parent,
                                "kde-system-monitorapplet" );
  }
}

kde-system-monitor-applet::kde-system-monitor-applet( const QString& configFile, Plasma::Type type,
...
kde-system-monitor-applet::~kde-system-monitor-applet()
{
  save();
...
int kde-system-monitor-applet::widthForHeight( int height ) const
{
  return ( (int) ( height * mSizeRatio + 0.5 ) * mDockCount );
}

int kde-system-monitor-applet::heightForWidth( int width ) const
{
  return ( (int) ( width * mSizeRatio + 0.5 ) * mDockCount );
}

void kde-system-monitor-applet::resizeEvent( QResizeEvent* )
{
  layout();
}
...
void kde-system-monitor-applet::applySettings()
{
...
void kde-system-monitor-applet::sensorDisplayModified( bool modified )
{
  if ( modified )
    save();
}

void kde-system-monitor-applet::layout()
{
...
int kde-system-monitor-applet::findDock( const QPoint& point )
{
  if ( orientation() == Qt::Horizontal )
...
void kde-system-monitor-applet::dragEnterEvent( QDragEnterEvent *e )
{
  e->setAccepted( Q3TextDrag::canDecode( e ) );
}

void kde-system-monitor-applet::dropEvent( QDropEvent *e )
{
  QString dragObject;
...
void kde-system-monitor-applet::customEvent( QCustomEvent *e )
{
...
void kde-system-monitor-applet::removeDisplay( KSGRD::SensorDisplay *display )
{
  for ( uint i = 0; i < mDockCount; ++i )
...
void kde-system-monitor-applet::resizeDocks( uint newDockCount )
{
...
bool kde-system-monitor-applet::load()
{
  KStandardDirs* kstd = KGlobal::dirs();
  kstd->addResourceType( "data", "share/apps/kde-system-monitor" );
  QString fileName = kstd->findResource( "data", "kde-system-monitor-applet.xml" );
  ...
  if ( doc.doctype().name() != "kde-system-monitor-applet" ) {
    KMessageBox::sorry( this, i18n( "The file %1 does not contain a valid applet "
                        "definition, which must have a document type 'kde-system-monitor-applet'." ,
                          fileName ) );
    return false;
  }
...
bool kde-system-monitor-applet::save()
{
  // Parse the XML file.
  QDomDocument doc( "kde-system-monitor-applet" );
  ...
  KStandardDirs* kstd = KGlobal::dirs();
  kstd->addResourceType( "data", "share/apps/kde-system-monitor" );
  QString fileName = kstd->saveLocation( "data", "kde-system-monitor" );
  fileName += "/kde-system-monitor-applet.xml";
...
void kde-system-monitor-applet::addEmptyDisplay( QWidget **dock, uint pos )
{
...
#include "kde-system-monitor-applet.moc"
