...
IconArea::IconArea(QGraphicsItem *parent)
:
...
bool IconArea::iconNamesVisible() const
{
    return m_iconNamesVisible;
}

void IconArea::setIconNamesVisible(bool enable)
{
...
bool IconArea::locked() const
{
    return m_locked;
}

void IconArea::setLocked(bool enable)
{
    m_locked = enable;
    setAcceptDrops(!enable);
}

IconGridLayout * IconArea::layout()
{
    return m_layout;
}

int IconArea::iconCount() const
{
    return m_icons.size();
}

void IconArea::insert(int index, const ItemData &itemData)
{
...
void IconArea::insert(int index, const QList<ItemData> &itemDataList)
{
...
void IconArea::removeAt(int index)
{
    if (m_dropMarkerIndex != -1 && m_dropMarkerIndex <= index) {
...
ItemData IconArea::iconAt(int index) const
{
    return m_icons.at(index)->itemData();
}

int IconArea::iconIndexAtPosition(const QPointF& pos) const
{
...
bool IconArea::eventFilter(QObject *watched, QEvent *event)
{
...
void IconArea::dragEnterEvent(QGraphicsSceneDragDropEvent *event)
{
    Q_ASSERT(!m_locked);
...
void IconArea::dragMoveEvent(QGraphicsSceneDragDropEvent *event)
{
...
void IconArea::dragLeaveEvent(QGraphicsSceneDragDropEvent *event)
{
    Q_UNUSED(event);
...
void IconArea::dropEvent(QGraphicsSceneDragDropEvent *event)
{
...
void IconArea::deletePlaceHolder() {
...
int IconArea::determineDropMarkerIndex(const QPointF &localPos) const
{
    if (m_placeHolder) {
        return 0;
    }
