...
// ctor, dtor
EnvCanadaIon::EnvCanadaIon(QObject *parent, const QStringList &args)
    ...
    while (!xml.atEnd()) {
        ...
        if (xml.isStartElement()) {
            if (xml.name() == "country") {
                ...
                data.territoryName = xml.readElementText();
...
void EnvCanadaIon::parsePrecipitationForecast(WeatherData::ForecastInfo *forecast, QXmlStreamReader& xml)
{
    ...
    while (!xml.atEnd()) {
        ...
        if (xml.isStartElement()) {
            //kDebug() << "parsePrecipitationForecast() ====> TAG: " << xml.name().toString() << endl;
            if (xml.name() == "textSummary") {
...
void EnvCanadaIon::setIonOption(int option, QVariant value)
{
    switch (option) {
...
void EnvCanadaIon::updateData()
{
    ...
    foreach(QString keyname, sources) {
        ...
        if (!this->validLocation(keyname)) {
            kDebug() << "EnvCanadaIon::updateData() " << keyname << " is not a valid location" << endl;
            //setData(keyname, I18N_NOOP("Invalid Place"), "Invalid location");
            continue;
        }
        setData(keyname, I18N_NOOP("Country"), this->country(keyname));
        setData(keyname, I18N_NOOP("Territory"), this->territory(keyname));
        setData(keyname, I18N_NOOP("Place"), this->city(keyname));
        setData(keyname, I18N_NOOP("Region"), this->region(keyname));
        setData(keyname, I18N_NOOP("Airport Code"), this->station(keyname));

        // Real weather - Current conditions
        setData(keyname, I18N_NOOP("Observations At"), this->observationTime(keyname));
        setData(keyname, I18N_NOOP("Current Conditions"), this->condition(keyname));
        dataFields = this->temperature(keyname);
        setData(keyname, I18N_NOOP("Temperature"), dataFields["temperature"]);
        ...
        if (dataFields["comfortTemperature"] != "N/A" && !dataFields["comfortTemperature"].isEmpty()) {
            if (dataFields["comfortTemperature"].toFloat() <= 0 || (dataFields["comfortTemperature"].toFloat() <= 32 && !d->m_useMetric)) {
                setData(keyname, I18N_NOOP("Windchill"), QString("%1%2").arg(dataFields["comfortTemperature"]).arg(QChar(176)));
            } else {
                setData(keyname, I18N_NOOP("Humidex"), QString("%1%2").arg(dataFields["comfortTemperature"]).arg(QChar(176)));
            }
        }

        setData(keyname, I18N_NOOP("Dewpoint"), this->dewpoint(keyname));
        dataFields = this->pressure(keyname);
        setData(keyname, I18N_NOOP("Pressure"), dataFields["pressure"]);
        setData(keyname, I18N_NOOP("Pressure Tendency"), dataFields["pressureTendency"]);
        setData(keyname, I18N_NOOP("Visibility"), this->visibility(keyname));
        setData(keyname, I18N_NOOP("Humidity"), this->humidity(keyname));
        dataFields = this->wind(keyname);
        setData(keyname, I18N_NOOP("Wind Speed"), dataFields["windSpeed"]);
        setData(keyname, I18N_NOOP("Wind Gust"), dataFields["windGust"]);
        setData(keyname, I18N_NOOP("Wind Direction"), dataFields["windDirection"]);

        dataFields = this->regionalTemperatures(keyname);
        setData(keyname, I18N_NOOP("Normal High"), dataFields["normalHigh"]);
        setData(keyname, I18N_NOOP("Normal Low"), dataFields["normalLow"]);


        // Check if UV index is available for the location
        dataFields = this->uvIndex(keyname);
        if (!dataFields["uvRating"].isEmpty()) {
            setData(keyname, I18N_NOOP("UV Rating"), dataFields["uvRating"]);
            setData(keyname, I18N_NOOP("UV Index"), dataFields["uvIndex"]);
        }
        ...
        if (!dataFields["watch"].isEmpty()) {
            ...
            setData(keyname, I18N_NOOP("Watch Priority"), fieldList[0]);
            setData(keyname, I18N_NOOP("Watch Description"), fieldList[1]);
            setData(keyname, I18N_NOOP("Watch Info"), fieldList[2]);
            setData(keyname, I18N_NOOP("Watch Timestamp"), fieldList[3]);
        }
        if (!dataFields["warning"].isEmpty()) {
            fieldList = dataFields["warning"].split('|');
            setData(keyname, I18N_NOOP("Warning Priority"), fieldList[0]);
            setData(keyname, I18N_NOOP("Warning Description"), fieldList[1]);
            setData(keyname, I18N_NOOP("Warning Info"), fieldList[2]);
            setData(keyname, I18N_NOOP("Warning Timestamp"), fieldList[3]);
        }
        ...
        foreach(QString forecastItem, forecastList) {
            ...
            if (d->m_useMetric) {
                setData(keyname, QString(I18N_NOOP("Short Forecast Day %1")).arg(i), QString("%1|%2|%3|%4|%5") \
                         .arg(fieldList[0]).arg(fieldList[1]).arg(fieldList[3]).arg(fieldList[4]).arg(fieldList[5]));

                setData(keyname, QString(I18N_NOOP("Long Forecast Day %1")).arg(i), QString("%1|%2|%3|%4|%5|%6|%7|%8") \
        ...
        setData(keyname, I18N_NOOP("Yesterday High"), dataFields["prevHigh"]);
        setData(keyname, I18N_NOOP("Yesterday Low"), dataFields["prevLow"]);
        setData(keyname, I18N_NOOP("Yesterday Precip Total"), dataFields["prevPrecip"]);

        dataFields = this->sunriseSet(keyname);
        setData(keyname, I18N_NOOP("Sunrise At"), dataFields["sunrise"]);
        setData(keyname, I18N_NOOP("Sunset At"), dataFields["sunset"]);

        dataFields = this->moonriseSet(keyname);
        setData(keyname, I18N_NOOP("Moonrise At"), dataFields["moonrise"]);
        setData(keyname, I18N_NOOP("Moonset At"), dataFields["moonset"]);

        dataFields = this->weatherRecords(keyname);
        setData(keyname, I18N_NOOP("Record High Temperature"), dataFields["recordHigh"]);
        setData(keyname, I18N_NOOP("Record Low Temperature"), dataFields["recordLow"]);
        setData(keyname, I18N_NOOP("Record Rainfall"), dataFields["recordRain"]);
        setData(keyname, I18N_NOOP("Record Snowfall"), dataFields["recordSnow"]);

        setData(keyname, I18N_NOOP("Ion"), "Environment Canada");
        i = 0;
    }
}
...
{
    return d->m_weatherData[key].territoryName;
}
...
QString EnvCanadaIon::dewpoint(QString key)
{
    if (d->m_useMetric) {
        if (!d->m_weatherData[key].dewpoint.isEmpty()) {
            return QString("%1%2%3").arg(QString::number(d->m_weatherData[key].dewpoint.toFloat(), 'f', 1)).arg(QChar(176)).arg("C");
        }
    }
 
    if (!d->m_weatherData[key].dewpoint.isEmpty()) {
         return QString("%1%2%3").arg(QString::number(d->m_formula.celsiusToF(d->m_weatherData[key].dewpoint.toFloat()), 'f', 1)).arg(QChar(176)).arg("F");
    }
...
QString EnvCanadaIon::visibility(QString key)
{
    if (!d->m_weatherData[key].visibility == 0) {
        if (d->m_useMetric) {
            return QString("%1%2").arg(QString::number(d->m_weatherData[key].visibility, 'f', 1)).arg("km"); 
        } else {
            return QString("%1 %2").arg(QString::number(d->m_formula.kilometersToMI(d->m_weatherData[key].visibility), 'f', 2)).arg("mi");
        }
    }
    return QString("N/A");
}
...
QMap<QString, QString> EnvCanadaIon::temperature(QString key)
{
    ...
    if (d->m_useMetric) {
        if (!d->m_weatherData[key].temperature.isEmpty()) {
            temperatureInfo.insert("temperature", QString("%1%2%3").arg(QString::number(d->m_weatherData[key].temperature.toFloat(), 'f', 1)).arg(QChar(176)).arg("C"));
        }
    }
    else {
        if (!d->m_weatherData[key].temperature.isEmpty()) {
            temperatureInfo.insert("temperature", QString("%1%2%3").arg(QString::number(d->m_formula.celsiusToF(d->m_weatherData[key].temperature.toFloat()), 'f', 1)).arg(QChar(176)).arg("F"));
        } else {
...
QMap<QString, QString> EnvCanadaIon::pressure(QString key)
{
    ...
    if (d->m_weatherData[key].pressure == 0) {
        ...
        if (d->m_useMetric) {
            pressureInfo.insert("pressure", QString("%1 %2").arg(QString::number(d->m_weatherData[key].pressure, 'f', 1)).arg("kPa"));
        } else {
            pressureInfo.insert("pressure", QString("%1 %2").arg(QString::number(d->m_formula.kilopascalsToInches(d->m_weatherData[key].pressure), 'f', 2)).arg("in"));
        }
...
QMap<QString, QString> EnvCanadaIon::wind(QString key)
{
    ...
    if (d->m_weatherData[key].windSpeed.isEmpty()) {
        ...
        if (d->m_useMetric) {
            windInfo.insert("windSpeed", QString("%1%2").arg(QString::number(d->m_weatherData[key].windSpeed.toInt())).arg("km/h"));
        } else {
            windInfo.insert("windSpeed", QString("%1%2").arg(QString::number(d->m_formula.kilometersToMI(d->m_weatherData[key].windSpeed.toInt()), 'f', 1)).arg("mph"));
        }
    }
    ...
    if (d->m_weatherData[key].windGust.isEmpty()) {
        ...
        if (d->m_useMetric) {
            windInfo.insert("windGust", QString("%1%2").arg(QString::number(d->m_weatherData[key].windGust.toInt())).arg("km/h"));
        } else {
            windInfo.insert("windGust", QString("%1%2").arg(QString::number(d->m_formula.kilometersToMI(d->m_weatherData[key].windGust.toInt()), 'f', 1)).arg("mph"));
        }
    }
...
QMap<QString, QString> EnvCanadaIon::regionalTemperatures(QString key)
{
    ...
    if (d->m_weatherData[key].normalHigh.isEmpty()) {
        ...
        if (d->m_useMetric) {
            regionalTempInfo.insert("normalHigh", QString("%1%2%3").arg(d->m_weatherData[key].normalHigh).arg(QChar(176)).arg("C"));
        } else {
            regionalTempInfo.insert("normalHigh", QString("%1%2%3").arg(d->m_formula.celsiusToF(d->m_weatherData[key].normalHigh.toFloat())).arg(QChar(176)).arg("F"));
        }
    }
    ...
    if (d->m_weatherData[key].normalLow.isEmpty()) {
        ...
        if (d->m_useMetric) {
            regionalTempInfo.insert("normalLow", QString("%1%2%3").arg(d->m_weatherData[key].normalLow).arg(QChar(176)).arg("C"));
        } else {
            regionalTempInfo.insert("normalLow", QString("%1%2%3").arg(d->m_formula.celsiusToF(d->m_weatherData[key].normalLow.toFloat())).arg(QChar(176)).arg("F"));
        }
    }
...
QMap<QString, QString> EnvCanadaIon::yesterdayWeather(QString key)
{
    ...
    if (d->m_weatherData[key].prevHigh.isEmpty()) {
        ...
        if (d->m_useMetric) {
            yesterdayInfo.insert("prevHigh", QString("%1%2%3").arg(d->m_weatherData[key].prevHigh).arg(QChar(176)).arg("C")); 
        } else {
            yesterdayInfo.insert("prevHigh", QString("%1%2%3").arg(QString::number(d->m_formula.celsiusToF(d->m_weatherData[key].prevHigh.toFloat()))).arg(QChar(176)).arg("F"));
        }
    }
    ...
    if (d->m_weatherData[key].prevLow.isEmpty()) {
        ...
        if (d->m_useMetric) {
            yesterdayInfo.insert("prevLow", QString("%1%2%3").arg(d->m_weatherData[key].prevLow).arg(QChar(176)).arg("C"));
        } else {
            yesterdayInfo.insert("prevLow", QString("%1%2%3").arg(QString::number(d->m_formula.celsiusToF(d->m_weatherData[key].prevLow.toFloat()), 'f', 1)).arg(QChar(176)).arg("F"));
        }
    }
    ...
    if (d->m_weatherData[key].prevPrecipTotal.isEmpty()) {
        ...
        if (d->m_useMetric) {
            yesterdayInfo.insert("prevPrecip", QString("%1%2").arg(d->m_weatherData[key].prevPrecipTotal).arg(d->m_weatherData[key].prevPrecipType));
        } else {
            yesterdayInfo.insert("prevPrecip", QString("%1 %2").arg(QString::number(d->m_formula.millimetersToIN(d->m_weatherData[key].prevPrecipTotal.toFloat()), 'f', 1)).arg("in"));
        }
    }
...
QMap<QString, QString> EnvCanadaIon::weatherRecords(QString key)
{
    ...
    if (d->m_weatherData[key].recordHigh == 0) {
        ...
        if (d->m_useMetric) {
            recordInfo.insert("recordHigh", QString("%1%2%3").arg(d->m_weatherData[key].recordHigh).arg(QChar(176)).arg("C"));
        } else {
            recordInfo.insert("recordHigh", QString("%1%2%3").arg(QString::number(d->m_formula.celsiusToF(d->m_weatherData[key].recordHigh), 'f', 1)).arg(QChar(176)).arg("F"));
        }
    }
    ...
    if (d->m_weatherData[key].recordLow == 0) {
        ...
        if (d->m_useMetric) {
            recordInfo.insert("recordLow", QString("%1%2%3").arg(d->m_weatherData[key].recordLow).arg(QChar(176)).arg("C"));
        } else {
            recordInfo.insert("recordLow", QString("%1%2%3").arg(QString::number(d->m_formula.celsiusToF(d->m_weatherData[key].recordLow), 'f', 1)).arg(QChar(176)).arg("F"));
        }
    ...
    if (d->m_weatherData[key].recordRain == 0) {
        ...
        if (d->m_useMetric) {
            recordInfo.insert("recordRain", QString("%1%2").arg(d->m_weatherData[key].recordRain).arg("mm"));
        } else {
            recordInfo.insert("recordRain", QString("%1 %2").arg(QString::number(d->m_formula.millimetersToIN(d->m_weatherData[key].recordRain), 'f', 1)).arg("in"));
        }
    }
    ...
    if (d->m_weatherData[key].recordSnow == 0) {
        ...
        if (d->m_useMetric) {
            recordInfo.insert("recordSnow", QString("%1%2").arg(d->m_weatherData[key].recordSnow).arg("cm"));
        } else {
            recordInfo.insert("recordSnow", QString("%1%2").arg(QString::number(d->m_formula.centimetersToIN(d->m_weatherData[key].recordSnow), 'f', 1)).arg("in"));
        }
    }
