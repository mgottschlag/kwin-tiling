...
// ctor, dtor
NOAAIon::NOAAIon(QObject *parent, const QStringList &args)
...
void NOAAIon::setIonOption(int option, QVariant value)
{
    switch (option) {
...
void NOAAIon::updateData()
{
    ...
    foreach(QString keyname, sources) {
        ...
        if (!this->validLocation(keyname)) {
            kDebug() << "NOAAIon::updateData() " << keyname << " is not a valid location" << endl;
            //setData(keyname, I18N_NOOP("Invalid Place"), "Invalid location");
            continue;
        }
        setData(keyname, I18N_NOOP("Country"), this->country(keyname));
        setData(keyname, I18N_NOOP("State"), this->state(keyname));
        setData(keyname, I18N_NOOP("Place"), this->place(keyname));
        ...
        if (dataFields["comfortTemperature"] != "N/A") {
           ...
           if (d->m_weatherData[keyname].heatindex_F != "NA" && d->m_weatherData[keyname].temperature_F.toInt() != d->m_weatherData[keyname].heatindex_F.toInt()) {
               setData(keyname, I18N_NOOP("Heat Index"), QString("%1%2").arg(dataFields["comfortTemperature"]).arg(QChar(176)));
           }
        }

        setData(keyname, I18N_NOOP("Dewpoint"), this->dewpoint(keyname));
        setData(keyname, I18N_NOOP("Pressure"), this->pressure(keyname));
        ...
        setData(keyname, I18N_NOOP("Ion"), "NOAA National Weather Service");
    }
}
...
QString NOAAIon::state(QString key)
{
    return d->m_weatherData[key].stateName;
}
...
{
    if (d->m_useMetric) {
        return QString("%1%2%3").arg(d->m_weatherData[key].dewpoint_C).arg(QChar(176)).arg("C");
    }
    return QString("%1%2%3").arg(d->m_weatherData[key].dewpoint_F).arg(QChar(176)).arg("F");
}
...
QString NOAAIon::visibility(QString key)
{
    ...
    if (d->m_useMetric) {
        return QString("%1 %2").arg(QString::number(d->m_formula.milesToKM(d->m_weatherData[key].visibility.toFloat()), 'f', 1)).arg("km");
    } 
    return QString("%1 %2").arg(d->m_weatherData[key].visibility).arg("mi");
}
...
QMap<QString, QString> NOAAIon::temperature(QString key)
{
    ...
    if (d->m_useMetric) {
        temperatureInfo.insert("temperature", QString("%1%2%3").arg(d->m_weatherData[key].temperature_C).arg(QChar(176)).arg("C"));
    } else {
        temperatureInfo.insert("temperature", QString("%1%2%3").arg(d->m_weatherData[key].temperature_F).arg(QChar(176)).arg("F"));
    }
...
QString NOAAIon::pressure(QString key)
{
    ...
    if (d->m_useMetric) {
        return QString("%1 %2").arg(QString::number(d->m_formula.inchesToKilopascals(d->m_weatherData[key].pressure.toFloat()), 'f', 1)).arg("kPa");
    } else {
        return QString("%1 %2").arg(d->m_weatherData[key].pressure).arg("in");
    }
}
...
QMap<QString, QString> NOAAIon::wind(QString key)
{
    ...
    if (d->m_weatherData[key].windSpeed == "NA") {
        ...
        if (d->m_useMetric) {
            windInfo.insert("windSpeed", QString("%1 %2").arg(QString::number(d->m_formula.milesToKM(d->m_weatherData[key].windSpeed.toFloat()), 'f', 1)).arg("km/h"));
        } else {
            windInfo.insert("windSpeed", QString("%1%2").arg(QString::number(d->m_weatherData[key].windSpeed.toFloat(), 'f', 1)).arg("mph"));
        }
    }
    ...
    if (d->m_weatherData[key].windGust == "NA") {
        ...
        if (d->m_useMetric) {
            windInfo.insert("windGust", QString("%1%2").arg(QString::number(d->m_formula.milesToKM(d->m_weatherData[key].windGust.toFloat()), 'f', 1)).arg("km/h"));
        } else {
            windInfo.insert("windGust", QString("%1%2").arg(QString::number(d->m_weatherData[key].windGust.toFloat(), 'f', 1)).arg("mph"));
        }
    }
