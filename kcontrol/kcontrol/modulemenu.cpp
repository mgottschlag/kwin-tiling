...
void ModuleMenu::fill(KMenu *parentMenu, const QString &parentPath)
{
      ...
      it != subMenus.end(); ++it)
  {
     ...
     parentMenu->insertItem(kapp->iconLoader()->loadIcon(group->icon(), K3Icon::Desktop, K3Icon::SizeSmall)
                        , name, menu);
  ...
  for (module=moduleList.first(); module != 0; module=moduleList.next())
  {
     ...
     int realid = parentMenu->insertItem(kapp->iconLoader()->loadIcon(module->icon(), K3Icon::Desktop, K3Icon::SizeSmall)
                                     , name, id);
