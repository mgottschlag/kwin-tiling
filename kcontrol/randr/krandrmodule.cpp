	...
	, m_changed(false)
{
	if (!isValid()) {
		...
		topLayout->addWidget(new QLabel(i18n("<qt>Your X server does not support resizing and rotating the display. Please update to version 4.3 or greater. You need the X Resize And Rotate extension (RANDR) version 1.1 or greater to use this feature. Error code: %1</qt>").arg(errorCode()), this));
		return;
	}
