...
KLorenzSaver::KLorenzSaver( Drawable drawable ) : kScreenSaver( drawable )
{
    readSettings();
...
void KLorenzSaver::drawOnce()
{
    ...
    for (int i=0; i<speed; i++) {
        ...
        // Choose a color
        XSetForeground(qt_xdisplay(), mGc,
            QColor((int)(sin(t*crate/pi)*127+128),
                   (int)(sin(t*crate/(pi-1))*127+128),
                   (int)(sin(t*crate/(pi-2))*127+128)).pixel() );
        // Draw a point
        XDrawPoint(qt_xdisplay(), mDrawable, mGc,
            (int)(kx*mWidth*1.5/kz)+(int)(mWidth/2),
            (int)(ky*mHeight*1.5/kz)+(int)(mHeight/2));
        t+=h;
    }
    if (++e>=epoch) newEpoch();
}
